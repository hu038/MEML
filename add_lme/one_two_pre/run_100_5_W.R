...
RMSE.nntanh=RMSE.svmkernel=RMSE.lm=RMSE.lme=RMSE.REEM=RMSE.tree=RMSE.bag=RMSE.rf=RMSE.boost=RMSE.svm=RMSE.nn=matrix(0,sim,4)
RMSE1.nntanh=RMSE1.svmkernel=RMSE1.lm=RMSE1.lme=RMSE1.REEM=RMSE1.tree=RMSE1.bag=RMSE1.rf=RMSE1.boost=RMSE1.svm=RMSE1.nn=matrix(0,sim,4)
...
for(s in 1:sim)
{
  ...
  for(j in 2:5){
    ...
    RMSE1.nntanh[s,j-1]=sqrt(sum((test$mu-a.nntanh$net.result)^2)/ni)
    ...
    RMSE.nn[s,j-1]=sqrt(sum((test$y-a.nn$net.result)^2)/ni)
    RMSE.nntanh[s,j-1]=sqrt(sum((test$y-a.nntanh$net.result)^2)/ni)
  }
  
  
}

save.image("data_100_5_W.RData")
