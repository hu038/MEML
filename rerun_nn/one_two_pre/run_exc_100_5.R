...
RMSE.svmkernel=RMSE.lm=RMSE.lme=RMSE.REEM=RMSE.tree=RMSE.bag=RMSE.rf=RMSE.boost=RMSE.svm=RMSE.nn=matrix(0,sim,4)
...
for(s in 1:sim)
{
  ...
  RMSE1.nn[s,j-1]=sqrt(sum((test$mu-a.nn$net.result)^2)/ni)
  ...
  RMSE.nn[s,j-1]=sqrt(sum((test$yi-a.nn$net.result)^2)/ni)
 }
