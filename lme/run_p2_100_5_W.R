...
for(s in 1:sim)
{
  ...
  for(j in 3:5){
    ...
    RMSE.merf[s,j-2]=sqrt(sum((test$y-a.merf)^2)/ni)
    ...
    RMSE.nn[s,j-2]=sqrt(sum((test$y-a.nn)^2)/ni)
    RMSE.nntanh[s,j-2]=sqrt(sum((test$y-a.nntanh)^2)/ni)
  }
  
}

save.image("data_p2_100_5_W.RData")
