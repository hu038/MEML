...
for(s in 1:sim)
{
  ...
  #fit.nn=neuralnet(y~x+t,train, hidden=2,linear.output = FALSE,stepmax = 1e6,threshold = 0.05, algorithm = "sag")
  #fit.nntanh=neuralnet(y~x+t,train,act.fct = "tanh", hidden=2,linear.output = FALSE,stepmax = 1e7,threshold = 0.05, algorithm = "sag")
